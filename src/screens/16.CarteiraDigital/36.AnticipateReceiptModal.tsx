...
// Espelha 32.WithdrawReceiptModal.tsx adaptado para antecipacoes
...
import { handleDownloadAnticipateReceipt, handleShareAnticipateWhatsApp, generateAnticipateTransactionId, generateAnticipateAuthCode } from './37.AnticipateReceiptCapture';
...
// Interface do modal de comprovante de antecipacao
interface AnticipateReceiptModalProps {
  visible: boolean; //..........Visibilidade do modal
  onClose: () => void; //.......Callback de fechar
  item: AnticipateHistoryItem | null; //..Item selecionado
}

// Modal de comprovante de antecipacao
// Exibe dados da antecipacao aprovada com opcoes de download e compartilhamento
const AnticipateReceiptModal: React.FC<AnticipateReceiptModalProps> = ({
  ...
  const formattedDate = formatReceiptDate(item.date); //..Data da antecipacao
  const formattedGrossAmount = formatCurrency(item.amount); //..Valor bruto
  const formattedNetAmount = formatCurrency(item.netAmount); //..Valor liquido
  const formattedDiscount = formatCurrency(item.amount - item.netAmount); //..Desconto
  ...
  const transactionId = generateAnticipateTransactionId(item); //..Id da transacao
  const authCode = generateAnticipateAuthCode(item); //..Codigo autenticacao
  ...
  const onDownload = async () => {
    ...
    try {
      await handleDownloadAnticipateReceipt(item); //..Baixa PNG
    } finally {
  ...
  const onShare = async () => {
    ...
    try {
      await handleShareAnticipateWhatsApp(item); //..Compartilha PNG
    } finally {
      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        ...
        <TouchableOpacity style={styles.receiptCard} activeOpacity={1} onPress={() => {}}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <Text style={styles.receiptTitle}>Comprovante de Antecipação</Text>
            ...
            <View style={styles.dataSection}>
              ...
              {/* Data da antecipacao */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Data da antecipação</Text>
                <Text style={styles.dataValue}>{formattedDate}</Text>
              </View>

              {/* Recebiveis antecipados */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Recebíveis antecipados</Text>
                <Text style={styles.dataValue}>{item.receivableCount}</Text>
              </View>

              {/* Valor bruto */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Valor bruto</Text>
                <Text style={styles.dataValue}>{formattedGrossAmount}</Text>
              </View>

              {/* Desconto */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Desconto</Text>
                <Text style={styles.dataValue}>- {formattedDiscount}</Text>
              </View>
              ...
              {/* Valor liquido recebido (destacado) */}
              <View style={styles.valueHighlight}>
                <Text style={styles.valueLabel}>Valor líquido recebido</Text>
                <Text style={styles.valueAmount}>{formattedNetAmount}</Text>
                <View style={styles.statusBadge}>
                  <Text style={styles.statusText}>Antecipação aprovada</Text>
                </View>
              </View>
...
// Estilos do modal de comprovante (identicos ao 32.WithdrawReceiptModal)
const styles = StyleSheet.create({
  ...
  // Container do valor liquido recebido
  valueHighlight: {
...
// Export padrao
export default AnticipateReceiptModal;
