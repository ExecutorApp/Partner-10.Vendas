...
import { handleDownloadReceipt, handleShareWhatsApp, generateTransactionId, getPaymentDate, generateAuthCode } from './25.ReceiptImageCapture';
...
// Interface do modal de comprovante
interface InstallmentReceiptModalProps {
  visible: boolean; //..........Visibilidade do modal
  onClose: () => void; //.......Callback de fechar
  installment: FullInstallment | null; //..Parcela selecionada
  clientName: string; //.........Nome do cliente
}

// Modal de comprovante de pagamento de parcela
// Exibe dados da parcela paga com opcoes de download e compartilhamento
const InstallmentReceiptModal: React.FC<InstallmentReceiptModalProps> = ({
  visible, onClose, installment, clientName,
}) => {
  ...
  // Aguarda fontes
  if (!fontsLoaded || !installment) return null;

  // Dados formatados do comprovante
  const formattedDueDate = formatDueDate(installment.dueDate); //..Data de vencimento
  const formattedPayDate = getPaymentDate(installment.dueDate); //..Data de pagamento
  const formattedAmount = formatCurrency(installment.amount); //...Valor formatado
  const transactionId = generateTransactionId(installment); //....Id da transacao
  const authCode = generateAuthCode(installment); //..............Codigo autenticacao
  ...
  const onDownload = async () => {
    ...
    try {
      await handleDownloadReceipt(installment, clientName); //..Baixa PNG
    } finally {
  ...
  const onShare = async () => {
    ...
    try {
      await handleShareWhatsApp(installment, clientName); //..Compartilha PNG
    } finally {
      ...
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose}>
        ...
        <TouchableOpacity style={styles.receiptCard} activeOpacity={1} onPress={() => {}}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <Text style={styles.receiptSubtitle}>Parcela {installment.installment}/{installment.totalInstallments}</Text>
            ...
            <View style={styles.dataSection}>
              ...
              <View style={styles.dataRow}>
                ...
                <Text style={styles.dataValue}>{installment.productName}</Text>
              </View>

              {/* Parcela */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Parcela</Text>
                <Text style={styles.dataValue}>{installment.installment} de {installment.totalInstallments}</Text>
              </View>

              {/* Vencimento */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Vencimento</Text>
                <Text style={styles.dataValue}>{formattedDueDate}</Text>
              </View>

              {/* Data de pagamento */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Data do pagamento</Text>
                <Text style={styles.dataValue}>{formattedPayDate}</Text>
              </View>

              {/* Metodo */}
              <View style={styles.dataRow}>
                <Text style={styles.dataLabel}>Método</Text>
                <Text style={styles.dataValue}>Débito automático</Text>
              </View>
